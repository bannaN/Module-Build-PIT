...
### unknown tests ###
{
  ### disallow unknowns ###
  {
    ...
    like( last_error(), qr/^Key 'foo' is not a valid key/, "   warning recorded ok" );
  }
  ...
  ### allow unknown ###
  {
    ...
    is_deeply( $rv, { foo => 42 }, "check call() with unknown args allowed" );
  }
}
...
### no_override tests ###
{
  ...
  like( last_error(), qr/^You are not allowed to override key/, "   warning recorded ok" );
}

### strict_type tests ###
{
  my @list = ( [ { strict_type => 1, default => [] }, 0 ], [ { default => [] }, 1 ], );
  ...
  for my $aref (@list) {
    ...
    ### proper value ###
    {
      ...
      is( ref $rv->{foo}, 'ARRAY', "   found provided value in rv" );
    }
    ...
    ### improper value ###
    {
      ...
      like( last_error(), qr/^Key 'foo' needs to be of type 'ARRAY'/, "   warning recorded ok" );
    }
  }
}
...
### required tests ###
{
  ...
  ### required value omitted ###
  {
    ...
    like( last_error, qr/^Required option 'foo' is not provided/, "   warning recorded ok" );
  }
}

### defined tests ###
{
  my @list = ( [ { defined => 1, default => 1 }, 0 ], [ { default => 1 }, 1 ], );
  ...
  for my $aref (@list) {
    ...
    ### value provided undefined ###
    {
      ...
      like( last_error, qr/^Key 'foo' must be defined when passed/, "   warning recorded ok" );
    }
  }
}
...
### warnings [rt.cpan.org #69626]
{
  ...
  like( $@, qr/expects two arguments/, "   error stored ok" );
}
...
### warnings fatal test
{
  ...
  ok( $@, "Call dies with fatal toggled" );
  like( $@, qr/invalid type/, "   error stored ok" );
}
...
### big template test ###
{
  ...
  my $tmpl = {
    ...
    gender    => { required => 1, allow   => [ qr/M/i, qr/F/i ], },
    married => { allow => [ 0, 1 ] },
    age     => { default => 21, allow       => qr/^\d+$/, },
    id_list => { default => [], strict_type => 1 },
    phone   => {
      allow => sub { 1 if +shift }
    },
    bureau => { default => 'NSA', no_override => 1 },
  };

  ### the args to send ###
  my $try = { firstname => 'joe', lastname => 'jackson', gender => 'M', married => 1, age => 21, id_list => [ 1 .. 3 ], phone => '555-8844', };
  ...
  is( $rv->{lastname}, $lastname, "   found provided values in rv" );
}
...
### $Params::Check::CALLER_DEPTH test
{
  ...
  like( last_error, qr/for .*::wrapper by .*::inner$/, "wrong caller without CALLER_DEPTH" );

  local $Params::Check::CALLER_DEPTH = 1;
  outer( { dummy => { required => 1 } }, {} );

  like( last_error, qr/for .*::inner by .*::outer$/, "right caller with CALLER_DEPTH" );
}
